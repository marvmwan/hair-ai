...
export const RevenueCatProvider: React.FC<RevenueCatProviderProps> = ({
  ...
  const checkSubscriptionStatus = async () => {
    try {
      ...
      const subscriptionStatus = await getItem(StorageKeys.IS_PAID);
  ...
  const purchase = async (): Promise<boolean> => {
    try {
      ...
      if (success) {
        await saveItem(StorageKeys.IS_PAID, "true");
  ...
  const restorePurchases = async () => {
    try {
      ...
      const hasActivePurchase = await getItem(StorageKeys.IS_PAID);
